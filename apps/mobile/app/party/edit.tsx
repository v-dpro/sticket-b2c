// Edit party — modal-feel form pushed from the party page's host tools.
// Prefills from GET /parties/:id, then PATCH /parties/:id (members get a
// party_update notification server-side). Hosts and co-hosts can edit;
// only the original host can cancel (that lives on the party page).
...
import { getParty, updateParty, type PartyVisibility } from '../../lib/api/parties';
...
export default function EditPartyScreen() {
  ...
  // Prefill from the live party (fresh beats params — a co-host may have
  // edited since the page was pushed).
  useEffect(() => {
    if (!id) return;
    let alive = true;
    getParty(id)
      .then((party) => {
        ...
        setStartsAt(party.startsAt ? new Date(party.startsAt) : null);
        setLoading(false);
      })
      .catch(() => {
        if (!alive) return;
  ...
  const canSubmit = title.trim().length > 0 && !submitting && !loading && !loadError;

  const seedDate = () => startsAt ?? new Date();
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      // Full-state PATCH: null clears the optional fields server-side.
    ...
    <View style={styles.screen}>
      ...
      <View style={{ paddingTop: insets.top }}>
        {/* No top-left X — the modal dismisses with swipe-down (grabber hints
            it); Cancel keeps an explicit abandon-the-edits affordance. */}
      ...
      >
        ...
        >
          <View style={styles.content}>
            <Text style={styles.eyebrow}>Edit party</Text>
            ...
            {loading ? (
              ...
              <Text style={styles.error}>Couldn&apos;t load this party — try again.</Text>
            ) : (
              <>
