...
export function generateAccessToken(userId: string, email: string): string {
  return jwt.sign({ userId, email, type: 'access' } satisfies TokenPayload, requireEnv('JWT_SECRET'), {
...
export function generateRefreshToken(userId: string, email: string): string {
  return jwt.sign({ userId, email, type: 'refresh' } satisfies TokenPayload, requireEnv('JWT_SECRET'), {
...
export async function verifyAppleToken(identityToken: string): Promise<{ email: string; appleUserId: string; name?: string }> {
  try {
...
export async function verifyGoogleToken(idToken: string): Promise<{ email: string; googleUserId: string; name?: string; picture?: string }> {
  try {
...
export async function handleGoogleSignIn(idToken: string): Promise<{ user: any; tokens: { accessToken: string; refreshToken: string } }> {
  ...
  if (user) {
    ...
    if (!user.googleId) updates.googleId = googleData.googleUserId;
...
export async function exchangeSpotifyCode(code: string): Promise<{ accessToken: string; refreshToken: string; expiresIn: number }> {
  try {
...
export async function getSpotifyProfile(accessToken: string): Promise<{ id: string; email: string; display_name: string; images: { url: string }[] }> {
  try {
