...
interface PresaleWindow {
  date?: string;
  time?: string;
  presales?: Array<{ type?: string; passwordType?: string; password?: string }>;
}
...
interface SOSEvent {
  ...
  presale_time?: string;
  presale_info?: string;
  presale_windows?: PresaleWindow[] | string;
  passcode?: string;
  citi_pass?: string;
  amex_pass?: string;
  fan_club_pass?: string;
  public_sale_date?: string;
...
function parsePresaleWindows(
  windows: PresaleWindow[] | string | undefined
): Array<{
  presaleType: string;
  ...
  const results: Array<{ presaleType: string; presaleStart: Date | null; code: string | null }> = [];
...
function getBestCode(event: SOSEvent): string | null {
  return event.passcode || event.citi_pass || event.amex_pass || event.fan_club_pass || null;
}

function getArtistName(event: SOSEvent): string {
...
function getTourName(event: SOSEvent): string | null {
  if (event.event_details) {
    const parts = event.event_details.split(/\s*[-–]\s*/);
...
async function fetchSOSEvents(): Promise<SOSEvent[]> {
  ...
  while (hasMore) {
    try {
      ...
      if (!response.ok) throw new Error(`SOS API error: ${response.status}`);

      const data: SOSResponse = await response.json();
      if (!data.success) throw new Error('SOS API returned success: false');
...
export async function syncPresalesFromERP(): Promise<number> {
  ...
  for (const event of events) {
    try {
      ...
      errors++;
      if (errors <= 3) console.error('[ERP Sync] Error:', event.id, error);
    }
  }
